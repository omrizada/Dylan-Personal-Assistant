---
last_updated:
confidence: low
source: template
---

# Stakeholders

## Key People

| Name | Role | Organization | Relationship | What They Care About |
|------|------|-------------|-------------|---------------------|
| *Run /onboard to populate* | | | | |

## Stakeholder Details

### [Person Name]
<!-- Copy this section for each key stakeholder -->

**Role**: [Their title/position]
**Organization**: [Company/team]
**Relationship**: [Manager / Direct Report / Peer / Client / Partner / Investor]
**Communication Style**: [How they prefer to communicate]
**What They Care About**: [Their priorities and concerns]
**How to Work With Them**: [Tips for effective interaction]
**Last Interaction**: [Date and context]

---

## Groups and Teams

| Group | Members | Purpose | Meeting Cadence |
|-------|---------|---------|----------------|
| *Run /onboard to populate* | | | |

## External Contacts

| Name | Organization | Context | Last Contact |
|------|-------------|---------|-------------|
| *Run /onboard to populate* | | | |
